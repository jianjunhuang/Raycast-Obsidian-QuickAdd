...
export default function Command() {
  ...
  async function handleSubmit(values: Preferences & { laterName: string; defaultDueDate: Date }) {
    try {
      ...
      const taskContent = `- [ ] ${values.laterName} 📅 ${formattedDueDate}`;
      ...
      console.log("Later added successfully!");
      showToast({ style: Toast.Style.Success, title: "Later added!", message: values.laterName });
    } catch (error) {
  ...
  return (
    <Form actions={<ActionPanel><Action.SubmitForm title="Add Later" onSubmit={handleSubmit} /></ActionPanel>} isLoading={isLoading}>
      <Form.TextField id="laterName" title="Content" placeholder="Enter task name" autoFocus />
      ...
      <Form.TextField id="taskPathTemplate" title="Later Path Template"  storeValue placeholder="e.g., Journal/Daily/{{DATE:YYYY}}/{{date}}" />
